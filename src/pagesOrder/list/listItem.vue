...
import { reqOrderList } from '@/services/order'
...
const getOrderList = async () => {
  const res = await reqOrderList(page.value, pageSize.value, props.orderState)
  if (res.code == 1) {
  ...
  <scroll-view scroll-y class="orders">
    <view class="card" v-for="item in OrderList" :key="item.id">
      ...
      <view class="status">
        ...
        <text class="icon-delete" v-if="[4, 5, 6].includes(item.orderState)"></text>
      </view>
